...
const getAuthHeaders = async (): Promise<Record<string, string>> => {
  try {
    ...
    console.log('🔑 Token retrieval for SDK:', { 
      hasToken: !!token, 
    ...
    if (token) {
      // Check what's available on the SDK
      console.log('� SDK Auth object:', {
        hasAuth: !!sdk.auth,
        authKeys: sdk.auth ? Object.getOwnPropertyNames(sdk.auth) : [],
        authMethods: sdk.auth ? Object.getOwnPropertyNames(Object.getPrototypeOf(sdk.auth)) : []
      });
      
      // Try setting token in SDK config
      if (sdk.config) {
        console.log('� SDK Config available:', Object.keys(sdk.config || {}));
      }
    ...
    console.error("❌ Error with SDK auth token:", error);
    return false;
  }
};

// Helper function to get authorization headers (fallback)
const getAuthHeaders = async (): Promise<Record<string, string>> => {
  try {
    const token = await SecureStore.getItemAsync("auth_token");
